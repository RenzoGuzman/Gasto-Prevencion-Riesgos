...
if zona:
    ...
    # Nota: distritos considerados como zona de influencia (desde la tabla de Breca)
    def _cap(txt):
    ...
    if seleccion == "Todas las empresas Breca":
        ...
        for emp in sorted(breca["EMPRESA"].unique()):
            ...
            items = ", ".join(f"{_cap(r['DISTRITO'])} - {_cap(r['REGION'])}"
                              for _, r in sub_e.iterrows())
            partes.append(f"{_cap(emp)} ({items})")
        nota = "Se consideraron como zona de influencia los distritos: " + "; ".join(partes) + "."
    else:
        sub_e = breca[breca["EMPRESA"] == seleccion]
        items = ", ".join(f"{_cap(r['DISTRITO'])} ({_cap(r['REGION'])})"
                          for _, r in sub_e.iterrows())
        nota = f"Se consideraron como zona de influencia los distritos: {items}."
