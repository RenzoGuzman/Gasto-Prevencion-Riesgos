...
if df is not None and "PLIEGO_NOMBRE" in df.columns:
    ...
    df.loc[_sin_pliego, "PLIEGO_NOMBRE"] = (
        "Municipalidad de " + df.loc[_sin_pliego, "DISTRITO_EJECUTORA_NOMBRE"].astype(str).str.strip().str.title()
    )
if df is None:
...
if "PRODUCTO_PROYECTO" in df_f.columns:
    ...
    if len(datos_pie):
        fig_pie = px.pie(
            ...
            color="Tipo",
            color_discrete_map={"Inversion (capital)": "#1565c0",
                                "Gasto corriente": "#ef6c00"},
        )
